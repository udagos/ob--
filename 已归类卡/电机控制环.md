...
actual_speed = (Capture_Count - Last_Count)\*  //**计算的电机的旋转速度**， 
Last_Count = Capture_Count
cont_val = PID_realize (==actual_speed==);   //进行 PID 计算
set_motor_speed (cont_val);// 设置 PWM 占空比
...
target_location = CIRCLE_PULSES;//电机转动一圈定时器能捕获到的编码器产生的脉冲个数
...
### 总结
电机码盘转了多少，计数器有计数
比如从开始位置设置20个，转了一圈；设置40，又转了一圈；设置0就反转回去两圈

根据速度和位置标黄，速度相当于只用捕获值做为pid输入，而速度还需要用捕获值计算速度作为pid输入
...
### 总结 
通过位置速度电流环，速度电流的上限处理，控制电流和速度最大有多少，也就是控制的期望
