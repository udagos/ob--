...
var TagFolderViewBase = class extends import_obsidian5.ItemView {
  ...
  showOrder(evt) {
    ...
    menu.addItem((item => {
      item.setTitle("标签").setIcon("hashtag").onClick((evt2 => {
    ...
    menu.addItem((item => {
      item.setTitle("项目").setIcon("document").onClick((evt2 => {
        const menu2 = new import_obsidian5.Menu;
  ...
  showLevelSelect(evt) {
    ...
    for (const level of [ 2, 3, 4, 5 ]) menu.addItem((item => {
      item.setTitle("第 " + (level - 1) +" 层" ).onClick((() => {
        setLevel(level);
      }));
  ...
  showMenu(evt, trail, targetTag, targetItems) {
    ...
    if (isTagTree) {
      ...
      if (navigator && navigator.clipboard) menu.addItem((item => item.setTitle(`复制标签:${expandedTags}`).setIcon("hashtag").onClick((async () => {
        await navigator.clipboard.writeText(expandedTags);
        new import_obsidian5.Notice("Copied");
      }))));
      menu.addItem((item => item.setTitle("新建笔记" + (targetTag ? " 在此" : "as like this")).setIcon("create-new").onClick((async () => {
        await this.plugin.createNewNote(trail);
      }))));
      if (targetTag) if (this.plugin.settings.useTagInfo && null != this.plugin.tagInfo) {
        const tag = targetTag;
        if (tag in this.plugin.tagInfo && "key" in this.plugin.tagInfo[tag]) menu.addItem((item => item.setTitle("解除固定").setIcon("pin").onClick((async () => {
          ...
          item.setTitle("固定").setIcon("pin").onClick((async () => {
        ...
        menu.addItem((item => {
          item.setTitle("设置替代标签").setIcon("pencil").onClick((async () => {
            var _a3;
        ...
        menu.addItem((item => {
          item.setTitle("更改标记").setIcon("pencil").onClick((async () => {
            var _a3;
        ...
        menu.addItem((item => {
          item.setTitle("将此标签重定向到...").setIcon("pencil").onClick((async () => {
            var _a3;
        ...
        if (targetItems) {
          menu.addItem((item => {
            item.setTitle("打开滚动视图").setIcon("sheets-in-box").onClick((async () => {
          ...
          menu.addItem((item => {
            item.setTitle("打开列表").setIcon("sheets-in-box").onClick((() => {
    ...
    if (!targetTag && targetItems && 1 == targetItems.length) {
      ...
      menu.addSeparator();
      menu.addItem((item => item.setTitle("新标签中打开").setSection("open").setIcon("lucide-file-plus").onClick((async () => {
        await this.app.workspace.openLinkText(path, path, "tab");
      }))));
      menu.addItem((item => item.setTitle("在右侧打开").setSection("open").setIcon("lucide-separator-vertical").onClick((async () => {
  ...
  getDisplayText() {
    return "tags" == this.treeViewType ? "标签文件夹" : "链接文件夹";
  }
  async onOpen() {
  ...
  onPaneMenu(menu, source2) {
    ...
    menu.addItem((item => {
      item.setIcon("pin").setTitle("固定").onClick((() => {
...
var TagFolderPlugin5 = class extends import_obsidian8.Plugin {
  ...
  display() {
    ...
    new import_obsidian8.Setting(containerEl).setName("始终打开").setDesc("将TagFolder放置在左侧窗格并在每次启动Obsidian时激活它").addToggle((toggle => toggle.setValue(this.plugin.settings.alwaysOpen).onChange((async value => {
      this.plugin.settings.alwaysOpen = value;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian8.Setting(containerEl).setName("使用固定").setDesc("当此功能启用时，固定信息将保存在下一个配置中设置的文件中。").addToggle((toggle => {
    ...
    const pi = new import_obsidian8.Setting(containerEl).setName("固定信息文件").setDisabled(!this.plugin.settings.useTagInfo).addText((text2 => {
    ...
    new import_obsidian8.Setting(containerEl).setName("禁用细化搜索").setDesc("当此功能启用时，相关标签将与标题一起显示，而不是创建子结构。").addToggle((toggle => {
    ...
    new import_obsidian8.Setting(containerEl).setName("显示方法").setDesc("如何显示文件标题").addDropdown((dropdown => dropdown.addOptions({
      "PATH/NAME": "PATH/NAME",
      NAME: "NAME",
      "NAME : PATH": "NAME : PATH"
    ...
    new import_obsidian8.Setting(containerEl).setName("排序方法").setDesc("如何排序项目").addDropdown((dd => {
    ...
    new import_obsidian8.Setting(containerEl).setName("优先显示不在子文件夹中的项目").setDesc("如果已启用此功能，则没有额外标签的项目将排在前面。").addToggle((toggle => {
      toggle.setValue(this.plugin.settings.sortExactFirst).onChange((async value => {
        this.plugin.settings.sortExactFirst = value;
        await this.plugin.saveSettings();
      }));
    }));
    new import_obsidian8.Setting(containerEl).setName("使用标题").setDesc("使用frontmatter或第一级标题中的值作为`NAME`。").addToggle((toggle => {
    ...
    const fpath = new import_obsidian8.Setting(containerEl).setName("Frontmatter 路径").setDisabled(!this.plugin.settings.useTitle).addText((text2 => {
    ...
    new import_obsidian8.Setting(containerEl).setName("排序方法").setDesc("如何排序标签").addDropdown((dd => {
    ...
    new import_obsidian8.Setting(containerEl).setName("使用虚拟标签").addToggle((toggle => {
    ...
    new import_obsidian8.Setting(containerEl).setName("在新笔记的frontmatter中存储标签").setDesc("否则，标签将存储在笔记顶部的#hashtags中。").addToggle((toggle => {
    ...
    new import_obsidian8.Setting(containerEl).setName("点击标签时在TagFolder内搜索标签").addToggle((toggle => {
    ...
    new import_obsidian8.Setting(containerEl).setName("在单独窗格中列出文件").addToggle((toggle => {
      toggle.setValue(this.plugin.settings.useMultiPaneList).onChange((async value => {
        this.plugin.settings.useMultiPaneList = value;
        await this.plugin.saveSettings();
      }));
    }));
    new import_obsidian8.Setting(containerEl).setName("显示列表在").setDesc("此选项适用于新打开的列表。").addDropdown((dropdown => {
    ...
    new import_obsidian8.Setting(containerEl).setName("隐藏项目").setDesc("隐藏着陆页或嵌套标签上的项目。").addDropdown((dd => {
    ...
    new import_obsidian8.Setting(containerEl).setName("合并冗余组合").setDesc("当此功能启用时，如果没有中间节点，a/b 和 b/a 将合并为 a/b。").addToggle((toggle => {
    ...
    new import_obsidian8.Setting(containerEl).setName("不简化空文件夹").setDesc("保留空文件夹，即使它们可以被简化。").addToggle((toggle => {
      toggle.setValue(this.plugin.settings.doNotSimplifyTags).onChange((async value => {
        this.plugin.settings.doNotSimplifyTags = value;
        await this.plugin.saveSettings();
      }));
    }));
    new import_obsidian8.Setting(containerEl).setName("不将嵌套标签视为专用级别").setDesc("将嵌套标签视为普通标签。").addToggle((toggle => {
      toggle.setValue(this.plugin.settings.disableNestedTags).onChange((async value => {
        this.plugin.settings.disableNestedTags = value;
        await this.plugin.saveSettings();
      }));
    }));
    new import_obsidian8.Setting(containerEl).setName("减少嵌套标签中的重复父级").setDesc("如果启用，#web/css, #web/javascript 将合并为 web -> css -> javascript。").addToggle((toggle => {
    ...
    new import_obsidian8.Setting(containerEl).setName("将未标记的项目保留在根目录").addToggle((toggle => {
    ...
    new import_obsidian8.Setting(containerEl).setName("使用传入链接").setDesc("").addToggle((toggle => toggle.setValue(this.plugin.settings.linkConfig.incoming.enabled).onChange((async value => {
      this.plugin.settings.linkConfig.incoming.enabled = value;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian8.Setting(containerEl).setName("使用传出链接").setDesc("").addToggle((toggle => toggle.setValue(this.plugin.settings.linkConfig.outgoing.enabled).onChange((async value => {
      this.plugin.settings.linkConfig.outgoing.enabled = value;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian8.Setting(containerEl).setName("隐藏间接链接的笔记").setDesc("").addToggle((toggle => toggle.setValue(this.plugin.settings.linkShowOnlyFDR).onChange((async value => {
      this.plugin.settings.linkShowOnlyFDR = value;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian8.Setting(containerEl).setName("连接链接树").setDesc("").addToggle((toggle => toggle.setValue(this.plugin.settings.linkCombineOtherTree).onChange((async value => {
    ...
    new import_obsidian8.Setting(containerEl).setName("目标文件夹").setDesc("如果配置了，插件将仅针对其中的文件。").addTextArea((text2 => text2.setValue(this.plugin.settings.targetFolders).setPlaceholder("学习,文档/摘要").onChange((async value => {
      this.plugin.settings.targetFolders = value;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian8.Setting(containerEl).setName("忽略文件夹").setDesc("忽略特定文件夹中的文档。").addTextArea((text2 => text2.setValue(this.plugin.settings.ignoreFolders).setPlaceholder("模板,列表/标准标签").onChange((async value => {
      this.plugin.settings.ignoreFolders = value;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian8.Setting(containerEl).setName("忽略笔记标签").setDesc("如果笔记包含以下标签之一，该笔记将被视为不存在。").addTextArea((text2 => text2.setValue(this.plugin.settings.ignoreDocTags).setPlaceholder("测试,测试1,测试2").onChange((async value => {
      this.plugin.settings.ignoreDocTags = value;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian8.Setting(containerEl).setName("忽略标签").setDesc("列表中的标签将被视为不存在。").addTextArea((text2 => text2.setValue(this.plugin.settings.ignoreTags).setPlaceholder("测试,测试1,测试2").onChange((async value => {
      this.plugin.settings.ignoreTags = value;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian8.Setting(containerEl).setName("存档标签").setDesc("如果配置了，具有这些标签的笔记将被移动到该标签下。").addTextArea((text2 => text2.setValue(this.plugin.settings.archiveTags).setPlaceholder("存档,停止使用").onChange((async value => {
    ...
    new import_obsidian8.Setting(containerEl).setName("标签扫描延迟").setDesc("设置反映元数据更改到标签树的延迟。(需要重新加载插件。)").addText((text2 => {
    ...
    new import_obsidian8.Setting(containerEl).setName("禁用拖拽标签").setDesc("`拖拽标签`使用内部API。如果出现问题，请暂时禁用然后重试。").addToggle((toggle => {
    ...
    new import_obsidian8.Setting(containerEl).setName("导出标签以便报告错误").setDesc("如果你想在GitHub上提交一个问题，这个信息可能会有用。另外，如果你想对标签名称保密，你可以使用`伪装`。").addButton((button => button.setButtonText("复制标签").setDisabled(false).onClick((async () => {
    ...
    })))).addButton((button => button.setButtonText("复制伪装后的标签").setDisabled(false).onClick((async () => {
      const x = new Map;
      let i = 0;
