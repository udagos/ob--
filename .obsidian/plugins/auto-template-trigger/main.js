...
// src/utils/templaterPluginUtils.ts
var TEMPLATER_PLUGIN_NAME = "templater-obsidian";
function shouldPreventTriggerIfTemplaterPluginUsed(plugin, file) {
  var _a, _b, _c, _d;
  const templaterPlugin = (_b = (_a = plugin.app.plugins) == null ? void 0 : _a.plugins) == null ? void 0 : _b[TEMPLATER_PLUGIN_NAME];
  if (!templaterPlugin) {
    return false;
  }
  ...
  const templaterFolders = (_d = templaterPlugin == null ? void 0 : templaterPlugin.settings) == null ? void 0 : _d.folder_templates.map(
    ({ folder }) => folder
  );
  if (!Array.isArray(templaterFolders)) {
    return false;
  }
  const isFileInTemplaterFolder = templaterFolders.some(
    (templaterFolder) => file.path.startsWith(templaterFolder + "/")
  );
...
var TextInputSuggest = class {
  ...
  async onInputChanged() {
    ...
    const suggestionPopupCloseTime = this.inputEl.dataset.closedAt ? parseFloat(this.inputEl.dataset.closedAt) : 0;
    const hasBeenClosedDuringAwait = suggestionPopupCloseTime && suggestionPopupCloseTime > getSuggestionsTime;
...
async function getTemplatesFolder(app2) {
  const templatesSettings = await app2.vault.readConfigJson("templates");
  return templatesSettings == null ? void 0 : templatesSettings.folder;
}
var getTemplateFiles = async (app2) => {
  const templatesFolder = await getTemplatesFolder(app2);
  if (!templatesFolder) {
    return [];
  }
  const templateFiles = app2.vault.getAllLoadedFiles().filter(
    (i) => i.path.startsWith(templatesFolder) && i.path.endsWith(".md") && i instanceof import_obsidian2.TFile
  );
...
var FolderSuggest = class extends TextInputSuggest {
  async getSuggestions(inputStr) {
    ...
    abstractFiles.forEach((folder) => {
      if (folder instanceof import_obsidian3.TFolder && folder.path.toLowerCase().contains(lowerCaseInputStr) && folder.path && folder.path !== templatesFolder) {
        folders.push(folder);
      }
    });
    return folders;
...
var Settings = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian4.Setting(containerEl).setName("Folder specific templates").setHeading().setDesc(
    ...
    this.plugin.settings.folderSpecificTemplates.map(
      ({ folderPath }, index) => {
        new import_obsidian4.Setting(this.containerEl).addSearch((cb) => {
          new FolderSuggest(this.app, cb.inputEl);
          cb.setValue(folderPath);
          cb.setPlaceholder("Select a folder");
          cb.onChange(() => {
            this.plugin.settings.folderSpecificTemplates[index].folderPath = cb.getValue();
          ...
          const templateFiles = this.app.vault.getAllLoadedFiles().filter((i) => i.path.startsWith(templatesFolder));
          templateFiles.forEach((file) => {
            const initialValue = this.plugin.settings.folderSpecificTemplates[index].templateName;
          ...
          cb.onChange((value) => {
            this.plugin.settings.folderSpecificTemplates[index].templateName = value;
            this.plugin.saveSettings();
          });
        }).addExtraButton((cb) => {
    ...
    new import_obsidian4.Setting(containerEl).addButton((cb) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Disable prompt").setDesc(
    ...
    new import_obsidian4.Setting(containerEl).setName("Debug logging").setDesc(
...
// src/main.ts
var TEMPLATE_SUGGESTION_CLASS = "suggestion-item";
var INSERT_TEMPLATE_COMMAND = "insert-template";
var AutoTemplatePromptPlugin = class extends import_obsidian5.Plugin {
  constructor() {
    super(...arguments);
    this.isReady = false;
  }
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("file-open", async (file) => {
        ...
        this.logGroup("[Auto Template Prompt] \u{1F4D9} " + file.name);
  ...
  async handleTemplateTrigger() {
    ...
    if (templateFiles.length > 1) {
      const mightHaveAssignedTemplate = this.settings.folderSpecificTemplates.length >= 0;
      const assignedTemplate = mightHaveAssignedTemplate ? this.findAssignedTemplate() : null;
      if (assignedTemplate) {
  ...
  findAssignedTemplate() {
    var _a;
    const currentFolder = (_a = this.app.workspace.getActiveFile()) == null ? void 0 : _a.parent;
    if (!currentFolder) {
    ...
    return assignedTemplateSetting == null ? void 0 : assignedTemplateSetting.templateName;
  }
  applySpecificTemplate(templateName) {
    setPromptOpacity(0);
    this.triggerTemplateSelectorPrompt();
