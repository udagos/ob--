...
var MIN_DAYS = 1;
var INITIAL_PROCESS_WAIT_MS = 10 * 1e3;
var PROCESS_INTERVAL_MS = 60 * 1e3;
var NUM_MS_IN_DAY = 864e5;
var AutoArchivePlugin = class extends import_obsidian3.Plugin {
  async onload() {
  ...
  getValidArchiveConfigs() {
    return this.settings.archiveConfigs.filter((config) => config.sourceFolder.trim().length > 0 && config.destFolder.trim().length > 0);
  }
  doesFileBelongToArchiveConfig(file, archiveConfig) {
  ...
  async processVault() {
    ...
    for (const archiveConfig of archiveConfigs) {
      ...
      if (sourceFolder instanceof import_obsidian3.TFolder) {
        ...
        console.warn(`Auto Archive: Configured source [${sourceFolder == null ? void 0 : sourceFolder.path}] is not a folder`);
      }
    }
  }
  /**
   ...
   * @param archiveConfig 
   */
  async processSourceFile(sourceFile, archiveConfig) {
  ...
  getMarkdownFilesInFolderRecursive(folder) {
    const files = [];
    for (const child of folder.children) {
      if (child instanceof import_obsidian3.TFile && child.name.endsWith(".md")) {
        files.push(child);
   ...
   */
  shouldFileBeArchived(sourceFile, archiveConfig) {
   ...
   * @param archiveConfig 
   */
  async deleteEmptyFolders(deletedFile, archiveConfig) {
    ...
    if (folderPathParts.length > 0) {
      ...
      for (let i = 0; i < numPathParts; i++) {
        ...
        if (abstractFolder instanceof import_obsidian3.TFolder && abstractFolder.children.length === 0) {
   ...
   * @returns file path
   */
  getFilePathInSourceFolder(sourceFile, archiveConfig) {
   ...
   * @returns folder path
   */
  getFolderFromPath(fileOrFolderPath) {
   ...
   * @param newFilePath 
   */
  async copyFile(file, newFilePath) {
    try {
   ...
   */
  async createFoldersInPath(fileOrFolderPath) {
    try {
...
var AutoArchiveSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    this.plugin.settings.archiveConfigs.forEach((config, i) => {
      ...
      new import_obsidian3.Setting(containerEl).setName("Source folder").setDesc("Archive notes from this folder").addSearch((cb) => {
        new FolderSuggest(cb.inputEl), cb.setPlaceholder("Example: work/notes").setValue(config.sourceFolder).onChange((newFolder) => {
          config.sourceFolder = newFolder;
          this.plugin.saveSettings();
        });
      }).setClass("aa-folder-search");
      new import_obsidian3.Setting(containerEl).setName("Archive folder").setDesc("Move notes to this folder").addSearch((cb) => {
        new FolderSuggest(cb.inputEl), cb.setPlaceholder("Example: work/note_archive").setValue(config.destFolder).onChange((newFolder) => {
          config.destFolder = newFolder;
          this.plugin.saveSettings();
        });
      }).setClass("aa-folder-search");
      new import_obsidian3.Setting(containerEl).setName("Maintain folder structure").setDesc("Maintain the source folder's hierarchy in the archive?").addToggle((cb) => {
        cb.setValue(config.maintainFolderStructure).onChange((value) => {
          config.maintainFolderStructure = value;
          this.plugin.saveSettings();
        });
      });
      new import_obsidian3.Setting(containerEl).setName("Auto-delete empty folders").setDesc("Delete folders from which all notes have been archived?").addToggle((cb) => {
        cb.setValue(config.deleteEmptyFolders).onChange((value) => {
          config.deleteEmptyFolders = value;
          this.plugin.saveSettings();
        });
      });
      new import_obsidian3.Setting(containerEl).setName("Archive after X days").setDesc("Age of note in days when archival will occur").addText((cb) => {
        cb.inputEl.addEventListener("blur", () => {
          cb.setValue(config.days.toString());
        });
        cb.setValue(config.days.toString()).onChange((value) => {
      ...
      new import_obsidian3.Setting(containerEl).addButton((cb) => {
        cb.setButtonText("Delete config").setClass("aa-delete-btn").onClick(() => {
    ...
    new import_obsidian3.Setting(containerEl).addButton((cb) => {
      cb.setButtonText("Add archive config").setCta().setClass("aa-new-config-btn").onClick(() => {
